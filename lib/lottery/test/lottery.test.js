...
/* function Lottery(uint _totalValue) { */
  ...
  it('Create Contract - constructor arguments', function* () {
    ...
    // state
    {
      ...
      assert.equal(lottery.ticketPrice, ticketPrice.toFixed(), 'ticketPrice');
      assert.equal(lottery.winnerAddress, 0, 'winnerAddress');
    }
    // query
    {
  ...
  it('rand tester', function* () {
    ...
    const noWay = yield contract.testRand(seed);
    ...
    const rand = yield contract.testRand(seed);
  ...
  //reach cap - close loterry
  it('find all open lotteries', function* () {
    const count = 10;
    const contracts = [];

    for (let i = 0; i < count; i++) {
      const ticketCount = 10+i;
      const ticketPrice = new BigNumber(10+i).mul(constants.ETHER);
      const contract = yield createContract(admin, ticketCount, ticketPrice);
      contracts.push(contract);
    }

    // query
   const result = yield lotteryJs.getOpen();
   const comparator = function(oA, oB) { return oA.address == oB.address; };
   const notFound = util.filter.isContained(contracts, result, comparator, true);
...
function* createContract(admin, ticketCount, ticketPrice) {
  ...
  const contract = yield lotteryJs.uploadContract(admin, args);
  const searchResult = yield lotteryJs.getLottery(contract.address); // blocks until found
  return contract;
}
